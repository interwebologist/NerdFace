"""Nerdface-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Nerdface
...
def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    ...
    The user identity is profile-scoped (same Nerdface profile = same userId).
    ...
    return {
        "user_id": f"nerdface_{user_digest}",
